...
public class GenericProjectile : BaseProjectile
{
    ...
    public GenericProjectile(string id, Player player, float lifeTime, Vector3 position, ItemRConfig config,
        int direction, ItemDescription item, bool isGrenade, ServerRConfig rConfig, ItemCatalog itemCatalog)
            ...
            null, false)
    {
        ...
        Collider = new AttackCollider(id, Position, new RectModel(0, 0, config.ProjectileWidth, config.ProjectileHeight), PrjPlane,
            player, item, item.ItemEffects, item.PrefabName, LifeTime, 0, player.Character.StatusEffects.HasEffect(ItemEffectType.Detect),
            itemCatalog);
